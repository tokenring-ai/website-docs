...
- **Hybrid Search**: Combines full-text search with token overlap scoring for better results
...
Advanced hybrid search tool combining full-text search and token overlap scoring with intelligent result merging.
...
1. **Parallel Search**: Executes both search and full-text search in parallel
2. **Token Overlap**: Computes token overlap score by counting query tokens found in each chunk
...
| `search()`           | query: string, limit: number       | Promise SearchResult array   | Search                     |
...
| `requireActiveProvider`          | agent: Agent                                     | FileIndexProvider            | Get current provider          |
...
for direct usage with TokenRingApp.
