...
- **Sandbox Isolation**: Optional sandbox execution for test isolation
...
This package does not define standalone tools. Testing functionality is accessed through chat commands and the TestingService.
...
|-------|------|----------|---------|-------------|
| agentDefaults | object | No | `{}` | Default configuration for agents |
| agentDefaults.maxAutoRepairs | number | No | `5` | Maximum number of auto-repair attempts |
| resources | record | No | `{}` | Testing resource definitions |

**Shell Command Resource Configuration:**

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| type | "shell" | Yes | - | Resource type identifier |
| name | string | Yes | - | Resource name |
| description | string | No | - | Resource description |
| workingDirectory | string | No | - | Working directory for command execution |
| command | string | Yes | - | Shell command to execute |
| isolation | "sandbox" \| "none" | No | `"sandbox"` | Execution isolation mode |
| timeoutSeconds | number | No | `120` | Test timeout in seconds |
| cropOutput | number | No | `10000` | Maximum output characters to display |

#### Environment Variables

This package does not define environment variables. Configuration is done through the plugin configuration.
...
├── hooks.ts                          # Hook exports (AfterTestsPassed)
├── hooks/
