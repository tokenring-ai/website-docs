...
The package provides 13 thinking tools for structured reasoning:

| Tool | Description |
|------|-------------|
| `scientific-method-reasoning` | Scientific method reasoning with hypothesis tracking |
| `socratic-dialogue` | Questions assumptions through structured inquiry |
| `design-thinking` | Human-centered design process |
| `root-cause-analysis` | 5 Whys methodology for finding fundamental causes |
| `swot-analysis` | Strategic planning through strengths, weaknesses, opportunities, threats |
| `pre-mortem` | Imagines failure to prevent it |
| `dialectical-reasoning` | Considers opposing views |
| `first-principles` | Breaks down to fundamental truths |
| `decision-matrix` | Structured multi-criteria decision making |
| `lateral-thinking` | Creative problem reframing |
| `agile-sprint` | Iterative development planning |
| `feynman-technique` | Learning through explanation |
...
```yaml
# No configuration needed for thinking package
```
