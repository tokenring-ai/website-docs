...
- Supports attachments via `allowRemoteAttachments: true`
...
- **AgentCommandService**: Registers all chat commands
- **ChatService**: Main chat service with all methods
- **Context Handlers**: For building chat requests
- **RPC Service**: For remote chat management endpoints
- **AgentLifecycleService**: For lifecycle hooks
...
Context handlers build the AI chat request by gathering relevant information from various sources. Each handler is an async generator that yields `ChatInputMessage` items.
...
import type { ChatToolConfig } from "@tokenring-ai/chat";
...
**ChatToolConfig Schema:**

```typescript
const ChatToolConfigSchema = z.object({
  ...
  /** Tool Input Schema */
  inputArguments: z.record(z.string(), z.object({
    description: z.string(),
...
const ChatAgentDefaultConfig = z.object({
  ...
  compaction: z.object({
    ...
    focus: z.string().default(/* default focus text */)
  }).prefault({}),
  context: z.object({
...
```typescript
type TokenRingToolDefinition<InputSchema extends AITool["inputSchema"]> = {
  name: string;
  displayName: string;
  description: string;
...
```typescript
const StoredChatCompactionSchema = z.object({
